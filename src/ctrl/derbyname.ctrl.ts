...
export type Derbyname = { _id: ObjectId, name: string, numRoster: string, email: string }
...
export const derbyNameController = {
  ...
  async POST(req: Request, env: ExportedHandlerType, App: Realm.App) {
    ...
    const { name: _name, numRoster: _numRoster, email: _email } = body
    ...
    const player = { name, numRoster, email, emailConfirmed: false }
